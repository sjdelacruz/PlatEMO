classdef CMaOEAIGDv2 < ALGORITHM
% <many> <real/binary/permutation>
% IGD based many-objective evolutionary algorithm focusing in handling
% constraints 
...
methods
        function main(Algorithm,Problem)
            ...
            Population = Problem.Initialization();
            [Population,Rank,Dis] = EnvironmentalSelectionv2(Population,W,Problem.N);

            %% Optimization
            while Algorithm.NotTerminated(Population)
                
                %Constrained binary tournament
                MatingPool = TournamentSelection(2,Problem.N,sum(max(0,Population.cons),2),Rank,min(Dis,[],2));
                Offspring  = OperatorGAhalf(Population(MatingPool),{1,30,1/Problem.D,20});
                [Population,Rank,Dis] = EnvironmentalSelectionv2([Population,Offspring],W,Problem.N);
            end
        end
    end
end
