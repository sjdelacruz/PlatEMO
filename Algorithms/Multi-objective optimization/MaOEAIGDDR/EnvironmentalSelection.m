function [Population,Rank,Dis] = EnvironmentalSelection(Population,W,N,c,beta,alfa,gen)
% The environmental selection of MaOEA/IGD with Dynamic penalty function
...
%--------------------------------------------------------------------------
    ...
    vals  = FitnessDynamic(Population.objs, Population.cons, c, beta, alfa, gen);
    [~,x]      = unique(round(vals*1e4)/1e4,'rows');
    ...
    for i = 1 : length(Population)
        ...
        %Change objects by the new fit considering constraints
        temp = repmat(vals(i,:),size(W,1),1);
